    ...
    <div className="job-view-main" onClick={closeModal}>
      {filteredData.map((item) => (
        <div
          ...
          className="job-view-container d-flex flex-column"
        >
          <div className="title-container d-flex justify-content-between">
            <div className="job-view-title">{item.title}
            </div>
            {item.submission_date &&
              calculateRemainingDays(item.end_date, item.submission_date) >
                0 && (
          ...
          <div className="job-view-status">
            Status:{" "}
            <span className={`job-view-status ${getStatusColor(item.status)}`}>
              {item.status}
            </span>
          </div>
          <div className="d-flex job-view-employee-container">
            <div className="employees-working">Employees Working:</div>
            <div className="employees-map-container">
              {item.employees &&
                item.employees.map((employee, index) => (
                  <div key={index} className="job-view-employees">
                    <img src={profileimage} /> {employee}
                  </div>
                ))}
            </div>
          </div>
          <div className="job-view-start_end d-flex">
            <div className="job-view-start-date">
              <span>Start Date:</span>
              {item.start_date}
            </div>
            <div className="job-view-end-date">
              <span>End Date:</span>
              {item.end_date}
            </div>
          </div>
